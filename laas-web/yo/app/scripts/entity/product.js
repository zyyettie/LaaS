LaaS.module('Entities', function(Entities, LaaS, Backbone, Marionette) {
    ...
    var baseUrl = LaaS.Util.Constants.APPCONTEXT+LaaS.Util.Constants.APIVERSION+'/products';
    ...
    LaaS.reqres.setHandler('product:entityByUrl', function(options) {
        var options = options || {url:baseUrl};
        var url = options.url || baseUrl;
        var product = $.Deferred();
        $.getJSON(url).done(function(data) {
            var data = data ? data : {};
            product.resolve(data);
        });
