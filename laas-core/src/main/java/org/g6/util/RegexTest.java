...
public class RegexTest {
    public static String[] samples = {
            ...
            " 22908( 23996) 07/16/2015 14:14:33  RTE D RADTRACE    140 [ 0] trigger.operator.check              start                     process      CPU(   63    171 )"
    };
    ...
    public static String[] regexes = {
            ...
            "^\\s*(\\d+)\\(\\s+(\\d+)\\)\\s+(\\d+/\\d+/\\d+\\s+\\d+:\\d+:\\d+)\\s+RTE D RADTRACE.+\\]\\s+([^\\s]+)\\s+([^\\s]+)\\s+([^\\s]+)"
    };
